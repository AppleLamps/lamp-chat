...
class ChatView {
    ...
    updateSystemPromptSelectorText() {
        if (!this.systemPromptSelectorBtn) return;

        const activePrompt = systemPromptService.getActiveSystemPrompt();
        const promptName = activePrompt ? activePrompt.name : 'Select Prompt';

        const icon = this.systemPromptSelectorBtn.querySelector('svg.icon');
        this.systemPromptSelectorBtn.textContent = promptName + ' ';
        if (icon) {
            this.systemPromptSelectorBtn.appendChild(icon);
        }
    }
    ...
    init() {
        ...
        this.updateSystemPromptSelectorText();
    ...
    renderSystemPromptDropdown() {
        if (!this.systemPromptDropdown) return;

        const systemPrompts = systemPromptService.getSystemPrompts();
        const activePrompt = systemPromptService.getActiveSystemPrompt();
        
        this.systemPromptDropdown.innerHTML = ''; // Clear existing content

        // Create built-in prompts section
        const builtInPrompts = systemPromptService.getBuiltInPrompts();
        if (Object.keys(builtInPrompts).length > 0) {
            const builtInSection = document.createElement('div');
            builtInSection.className = 'prompt-provider-section';

            const builtInHeader = document.createElement('div');
            builtInHeader.className = 'prompt-provider-header';
            builtInHeader.innerHTML = `
                <div class="provider-icon builtin">🔧</div>
                BUILT-IN PROMPTS
            `;
            builtInSection.appendChild(builtInHeader);

            for (const [id, prompt] of Object.entries(builtInPrompts)) {
                const button = this.createSystemPromptButton(id, prompt, activePrompt);
                builtInSection.appendChild(button);
            }

            this.systemPromptDropdown.appendChild(builtInSection);
        }

        // Create custom prompts section
        const customPrompts = systemPromptService.getCustomPrompts();
        if (Object.keys(customPrompts).length > 0) {
            const customSection = document.createElement('div');
            customSection.className = 'prompt-provider-section';

            const customHeader = document.createElement('div');
            customHeader.className = 'prompt-provider-header';
            customHeader.innerHTML = `
                <div class="provider-icon custom">✨</div>
                CUSTOM PROMPTS
            `;
            ...
            this.systemPromptDropdown.appendChild(customSection);
        }

        // Add "Manage Prompts" button
        const manageSection = document.createElement('div');
        manageSection.className = 'prompt-provider-section';
        
        const manageButton = document.createElement('button');
        manageButton.className = 'prompt-manage-btn';
        manageButton.innerHTML = `
            <div class="prompt-info">
                <div class="prompt-name">⚙️ Manage Prompts</div>
                <div class="prompt-description">Create, edit, and delete system prompts</div>
            </div>
        `;
        ...
        manageButton.addEventListener('click', (e) => {
            e.stopPropagation();
            this.systemPromptDropdown.classList.remove('show');
            // Navigate to settings page with system prompts tab
            window.location.href = 'settings.html#system-prompts';
        });

        manageSection.appendChild(manageButton);
        this.systemPromptDropdown.appendChild(manageSection);
    }
    ...
    createSystemPromptButton(id, prompt, activePrompt) {
        const button = document.createElement('button');
        button.className = 'prompt-select-btn';
        ...
        const truncatedPrompt = prompt.prompt.length > 100 
            ...
            : prompt.prompt;
        promptDesc.textContent = truncatedPrompt;
        promptInfo.appendChild(promptDesc);

        button.appendChild(promptInfo);

        button.addEventListener('click', (e) => {
            e.stopPropagation();

            try {
                systemPromptService.setActiveSystemPrompt(id);
                this.updateSystemPromptSelectorText();
                this.systemPromptDropdown.classList.remove('show');
                
                // Re-render dropdown to update active state
                this.renderSystemPromptDropdown();
                
            } catch (error) {
                console.error("Error setting active system prompt:", error);
                alert(`Failed to select system prompt: ${error.message}`);
            }
        });
